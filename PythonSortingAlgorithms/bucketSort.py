...
def bucket_sort(data, ascending=True):
    #empty lists for each bucket
    buckets = [[] for i in range(10)]  
    #distribute numbers into buckets based on their range
    for num in data:
        index = num // 10
        buckets[index].append(num)
    
    result = []
    #sort each bucket using the insert_sort function
    for bucket in buckets:
        insert_sort(bucket, ascending)
         #combine the sorted buckets into the final result list
        if ascending:
...
#ascending sort
sorted_data_ascending = bucket_sort(data, ascending=True)
print("Sorted List Ascending:", sorted_data_ascending)

#descending sort
sorted_data_descending = bucket_sort(data, ascending=False)
